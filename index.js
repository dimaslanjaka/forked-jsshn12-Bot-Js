  ...
  client.onAddedToGroup((chat) => {
    ...
    if (totalMem < 257) {
      client
        .sendText(
          chat.id,
          `Halo warga ${name} mohon maaf, jika mau menginvite bot, silahkan membeli akses premium bot melalui wa.me//6281374347075`
        )
  ...
  client.onAddedToGroup(async (chat) => {
    ...
    if (groups.length > groupLimit) {
      await client
        .sendText(
          chat.id,
          `Sorry, bot tidak bisa join di grub untuk menginvite bot silahkan hubungi owner bot https://wa.me/ max group: ${groupLimit}`
        )
        .then(() => {
      ...
      if (chat.groupMetadata.participants.length < memberLimit) {
        await client
          .sendText(
            chat.id,
            `Sorry, BOT comes out if the group members do not exceed ${memberLimit} people`
          )
          .then(() => {
            client.leaveGroup(chat.id);
            client.deleteChat(chat.id);
        ...
        await client.simulateTyping(chat.id, true).then(async () => {
          await client.sendText(
            chat.id,
            `Hai minna~, Im ITEUNG V2 BOT. To find out the commands on this bot type #menu`
          );
        });
      }
    }
